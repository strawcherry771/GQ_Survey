...
const ResultPage = ({ answers, name, position }) => {
  ...
  const [docId, setDocId] = useState(null);  // Firestore 문서 ID
  const [feedbackScore, setFeedbackScore] = useState(null);  // 피드백 점수
  const [feedbackText, setFeedbackText] = useState('');  // 자유 피드백 텍스트
  ...
  useEffect(() => {
    const saveInitialResponse = async () => {
      try {
        ...
        setDocId(docRef.id); // 저장된 문서 ID 기억
        console.log('✅ 설문 응답 저장 완료:', docRef.id);
      } catch (error) {
  ...
  return (
    ...
    <div style={{ marginTop: '3rem' }}>
      <h3>🗣️ 이번 테스트는 어땠나요?</h3>

      <p style={{ marginBottom: '0.5rem' }}>만족도를 1~5점으로 평가해주세요:</p>
      ...
      <textarea
        placeholder="피드백이 있다면 적어주세요 (선택 사항)"
        value={feedbackText}
